...
WIDTH = 600
HEIGHT = 600
snake_head_bigger = 60
snake_body_bigger = 20
apple_bigger = 50

SQRS = 60
SQR_LEN = WIDTH / SQRS
COEFF = 0.3  # the higher, the easier to eat an apple
...
class Background(tk.Canvas):
    def __init__(self, master):
        ...
        self.snake_imgs = {"Up": self.up_snake_img, "Down": self.down_snake_img,
                           ...
                           "w": self.up_snake_img, "s": self.down_snake_img,
                           "a": self.left_snake_img, "d": self.right_snake_img}
    ...
    def binder(self):
        ...
        root.bind("<w>", lambda event: key_press(event, self, self.head))
        root.bind("<Down>", lambda event: key_press(event, self, self.head))
        root.bind("<s>", lambda event: key_press(event, self, self.head))
        root.bind("<Left>", lambda event: key_press(event, self, self.head))
        root.bind("<a>", lambda event: key_press(event, self, self.head))
        root.bind("<Right>", lambda event: key_press(event, self, self.head))
        root.bind("<d>", lambda event: key_press(event, self, self.head))
...
def key_press(event: tk.Event, canvas: tk.Canvas, head: int):
    ...
    if event.keysym in ["Up", "Down", "Left", "Right", "w", "s", "a", "d"]:
...
def move(canvas: tk.Canvas, head: int):
    ...
    if apple_coords[0] - COEFF * SQR_LEN <= head_coords[0] <= apple_coords[0] + COEFF * SQR_LEN and apple_coords[
        1] - COEFF * SQR_LEN <= head_coords[1] <= apple_coords[1] + COEFF * SQR_LEN:
        just_ate_apple = True
        canvas.delete(apple)
        apple_gen(canvas)
...
def apple_gen(canvas: tk.Canvas):
    ...
    while True:
        ...
        if not [rand_x + 1, rand_y + 1] in snake_coords:
            break
